...
from utils import find_closest_string, find_files, glob_file_in_folder

Category = dict[str, dict[str, str | dict["Category"]]]


def get_category_level_1(string: str):
    items = list(category.keys())
    idx = find_closest_string(string, items)
    return items[idx], category[items[idx]].get("level")


def get_category_level_2(level_1: str, string: str):
...
async def main():
    for _, row in data.iterrows():
        ...
        level_2_name, level_2 = get_category_level_2(level_1_name, category2)
        ...
        # get image
        image_folder = [i for i in image_folder_list if i.name.startswith(str(ids))][-1]
        ...
        loguru.logger.warning(
            ...
            + f'details: {'\n'.join(['/'.join(i.parts[2:]) for i in details])}'
        )
        loguru.logger.warning(
...
if __name__ == "__main__":
    ...
    image_path = "企悦汇选品1038-1197 (2)/未命名文件夹 2"
    image_folder_list = [i for i in pathlib.Path(image_path).iterdir() if i.is_dir()]
